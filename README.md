...
- **대기 전체 갱신 시작**은 Plexmate 제한이 `0`이고 Plex가 `NORMAL`일 때만 실행됩니다. 한 항목씩 설정한 간격으로 요청하며, 각 요청 뒤 Plex가 잠시 `BUSY`이면 다음 요청을 보내지 않고 최대 5분간 안정화를 기다립니다. 정체·마이그레이션·통신 장애·요청 오류일 때만 중지합니다.
